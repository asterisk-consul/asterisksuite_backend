/** Registra permisos de importación/exportación sin modificar roles. */
...
if (!tenant) {
  console.error('Uso: npx tsx prisma/seeds/data-transfer-permissions.seed.ts <tenant>')
  process.exit(1)
}
...
const permissions = RBAC_PERMISSIONS.filter(({ code }) => code.endsWith('.import') || code.endsWith('.export'))
...
async function main() {
  ...
  try {
    ...
    console.log(`${permissions.length} permisos de importación/exportación registrados en ${tenant}_db. No se modificaron roles.`)
  } finally {
    await pool.end()
  }
}
