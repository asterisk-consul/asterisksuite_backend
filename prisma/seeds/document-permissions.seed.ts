/** Registra el catálogo granular sin modificar asignaciones de roles existentes. */
...
if (!tenant) {
  console.error('Uso: npx tsx prisma/seeds/document-permissions.seed.ts <tenant>')
  process.exit(1)
}
...
const permissions = RBAC_PERMISSIONS.filter(({ code }) =>
...
async function main() {
  ...
  try {
    ...
    console.log(`${permissions.length} permisos documentales registrados en ${tenant}_db. No se modificaron roles.`)
  } finally {
    await pool.end()
  }
}
