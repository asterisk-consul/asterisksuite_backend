  ...
  async resolveItemWithTaxes(
    ...
    if (overrideUnitPrice === undefined) {
      if (product.is_rate_type) {
        throw new BadRequestException('Productos rate_type requieren overrideUnitPrice');
      }

      const resolved = await this.resolveProductPrice(productId, targetCurrencyCode);
