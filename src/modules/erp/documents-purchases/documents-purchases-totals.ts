...
import { ItemInput, TaxInput } from '../documents-sales/interfaces/item-input.interface';

import { CalculatedTotals } from '../documents-sales/interfaces/calculated-totals.interface';

@Injectable()
export class DocumentsPurchasesTotalsService {
  calculate(items: ItemInput[]): CalculatedTotals {
    const subtotal = round2(items.reduce((acc, i) => acc + i.price, 0));

    const exemptAmount = round2(items.reduce((acc, i) => acc + i.exempt_amount, 0));

    const taxableBase = round2(items.reduce((acc, i) => acc + i.taxable_base, 0));
    ...
    for (const item of items) {
      for (const tax of item.taxes) {
        ...
        if (existing) {
          ...
          existing.taxable_base = round2(existing.taxable_base + item.taxable_base);
        } else {
