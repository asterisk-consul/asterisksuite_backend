...
export class EngineeringValidationService {
  ...
  async validateNoCircularReference(parentProductId: string, childProductId: string) {
    if (parentProductId === childProductId) {
      throw new BadRequestException('Un producto no puede referenciarse a sí mismo');
    }
  ...
  private async existsPath(currentProductId: string, targetProductId: string): Promise<boolean> {
    ...
    for (const child of children) {
      ...
      const nested = await this.existsPath(child.child_product_id, targetProductId);
